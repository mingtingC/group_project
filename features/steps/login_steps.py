...
@then(u'she should see the email field')
def see_username_field(context):
    flaskr_found = re.search("email", context.browser.page_source, re.IGNORECASE)
    assert flaskr_found
...
@when(u'she logs in with username "{username}" and email "{email}" and password "{password}"')
def login(context, username, email,password):
    context.browser.get(context.server_address + "/login")
...
@then(u'she should return to the home page')
...
@given(u'a user visits the home page')
