...
from features.steps.login_utils import login
...
def see_login(ctx):
    login_found = re.search("login", ctx.browser.page_source, re.IGNORECASE)
    assert login_found
...
def step_impl(ctx):
    logout_found = re.search("logout", ctx.browser.page_source, re.IGNORECASE)
    assert logout_found

@then(u'she should see the Register link')
def step_impl(ctx):
    signup_found = ctx.browser.find_element_by_link_text("Sign up")
    assert signup_found